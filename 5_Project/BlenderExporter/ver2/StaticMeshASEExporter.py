...
# **********************************
bl_info = {
    "name": "StaticMesh ASE Exporter",
    "author": "Sin,SeongHyeon",
    "version": (0, 10, 0),
    "blender": (2, 90, 0),
    "api": 36080,
    "location": "File > Export > StaticMesh ASE Export(.ase)",
    "description": "StaticMesh ASE Export(.ase)",
        ...
        for object in bpy.context.selected_objects:
            ...
            if collisionObject(object) == True:
                ...
                for slot in object.material_slots:
                    if str(type(slot.material)) == "<class 'NoneType'>":
                        continue
...
class cMesh:
    def __init__(self, object):
        ...
        if str(type(object)) == "<class 'bpy_types.PoseBone'>":
            ...
            self.numcvfaces = ''
            self.cfacelist = ''
        elif str(type(object)) != "<class 'NoneType'>":
            if collisionObject(object) == False:
                ...
                if str(type(object.data.uv_layer_stencil)) != "<class 'NoneType'>":
    ...
    def __init__(self, object):
        ...
        if str(type(object)) == "<class 'bpy_types.PoseBone'>":
            ...
            for face in object.data.polygons:
                if optionAllowMultiMats:
                    if (collisionObject(object) == False):
                        self.matid = 0#matList.index(object.material_slots[face.material_index].material.name)
                    else:
                        self.matid = 0
                else:
    ...
    def __init__(self, object):
        ...
        for index, face in enumerate(object.data.loop_triangles):
            if len(object.data.uv_layers) == 0:
                temp = "Error:  No UV texture data for  + object.name"
...
class cNormal:
    ...
    def __repr__(self):
        return '''\t\t\t*MESH_FACENORMAL {0} {1} {2} {3}\n\t\t\t\t*MESH_VERTEXNORMAL {4} {5} {6} {7}\n\t\t\t\t*MESH_VERTEXNORMAL {8} {9} {10} {11}\n\t\t\t\t*MESH_VERTEXNORMAL {12} {13} {14} {15}\n'''.format(self.faceindex, self.facenormal[0], self.facenormal[1], self.facenormal[2], self.vertnormals[0][0], self.facenormal[0], self.facenormal[1], self.facenormal[2], self.vertnormals[1][0], self.facenormal[0], self.facenormal[1], self.facenormal[2], self.vertnormals[2][0], self.facenormal[0], self.facenormal[1], self.facenormal[2])
...
class cBoneObject:
    def __init__(self, object):
        ...
        for pbone in object.pose.bones:
            ...
            self.tAnimation = cAnimation(pbone)
            self.tAnimation_str = self.tAnimation
            if optionAllowMultiMats:
                self.material_ref = 0
            else:
                self.material_ref = currentMatId
...
class ExportAse(bpy.types.Operator, ExportHelper):
    ...
    bl_label = "Export"
    __doc__ = "StaticMesh ASE Export(.ase)"
    filename_ext = ".ase"
...
def menu_func(self, context):
    self.layout.operator(ExportAse.bl_idname, text = "StaticMesh ASE Export(.ase)")
