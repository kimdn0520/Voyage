...
#pragma region UI
	...
	mainTalkPanel->AddComponent<YouthText>();
	...
	spacePanel->GetComponent<Panel>()->SetOffset(
		GameProcess::_windowInfo.width / 2 - 200,
		GameProcess::_windowInfo.width / 2 + 200,
		GameProcess::_windowInfo.height / 2 - 440,
		GameProcess::_windowInfo.height / 2 + 440);
	...
	// Arrow
	shared_ptr arrowPanel = make_shared<GameObject>();
	arrowPanel->SetName("spacePanel");
	arrowPanel->AddComponent<Transform>();
	arrowPanel->AddComponent<Panel>();
	arrowPanel->GetComponent<Panel>()->SetOffset(
		GameProcess::_windowInfo.width / 2 - 300,
		GameProcess::_windowInfo.width / 2 -100,
		GameProcess::_windowInfo.height / 2 - 170,
		GameProcess::_windowInfo.height / 2 + 50);
	arrowPanel->GetComponent<Panel>()->SetImages(L"Arrow.png");
	arrowPanel->GetComponent<Panel>()->SetSortLayer(0);
	arrowPanel->SetActive(false);
	AddGameObject(arrowPanel);
	EventMachine::GetInstance()->SetArrowPanel(arrowPanel);
