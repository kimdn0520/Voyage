...
void AdulthoodScene::Init()
{
	if (!DataManager::GetInstance()->isDataLoad)
		DataManager::GetInstance()->DataLoad();

	EventMachine::GetInstance()->isAdulthoodIntro = true;
	
	EventMachine::GetInstance()->ChangeState(EventMachine::ADULTHOOD_INTRO);
	...
	Resources::GetInstance()->SetCubeMapTexture(skyBox->GetComponent<MeshRenderer>()->GetMeshInfo()->materialID, L"cloud.dds");
	AddGameObject(skyBox);
	EventMachine::GetInstance()->SetSkyBox(skyBox);
	...
	/*shared_ptr<LightningPrefab> lightningPrefab = make_shared<LightningPrefab>(Vector3(0.f, 10.f, 18.f));
	AddGameObject(lightningPrefab->gameObject);*/

	/*shared_ptr<SwirlPrefab> swirlPrefab = make_shared<SwirlPrefab>(Vector3(20.f, 4.f, 18.f));
	AddGameObject(swirlPrefab->gameObject);*/
...
#pragma region UI
	...
	// 이벤트에서 쓰일 Yes 버튼
	shared_ptr<GameObject> yesButton = make_shared<GameObject>();
	yesButton->SetName("YesBtn");
	yesButton->AddComponent<Transform>();
	yesButton->AddComponent<Button>();
	yesButton->GetComponent<Button>()->SetSortLayer(1);
	yesButton->GetComponent<Button>()->SetOffset(
		GameProcess::_windowInfo.width / 2 + 350.f,
		GameProcess::_windowInfo.width / 2 + 550.f,
		GameProcess::_windowInfo.height / 2 + 340.f,
		GameProcess::_windowInfo.height / 2 + 440.f);
	yesButton->GetComponent<Button>()->screenResize = Vector2(1920.f, 1080.f);
	yesButton->GetComponent<Button>()->Resize(GameProcess::resizeInfo.width, GameProcess::resizeInfo.height);
	yesButton->GetComponent<Button>()->SetImages((int)ButtonStatus::NORMAL, L"pngwingcom.png");
	yesButton->GetComponent<Button>()->SetImages((int)ButtonStatus::HIGHLIGHT, L"pngwingcom.png");
	yesButton->GetComponent<Button>()->SetImages((int)ButtonStatus::PRESSED, L"pngwingcom.png");
	yesButton->GetComponent<Button>()->SetIsAlpha(true);
	yesButton->GetComponent<Button>()->SetAlpha(0.f);
	yesButton->SetActive(false);
	AddGameObject(yesButton);
	EventMachine::GetInstance()->SetYesButton(yesButton);

	// 이벤트에서 쓰일 No 버튼
	shared_ptr<GameObject> noButton = make_shared<GameObject>();
	noButton->SetName("noBtn");
	noButton->AddComponent<Transform>();
	noButton->AddComponent<Button>();
	noButton->GetComponent<Button>()->SetSortLayer(1);
	noButton->GetComponent<Button>()->SetOffset(
		GameProcess::_windowInfo.width / 2 + 350.f,
		GameProcess::_windowInfo.width / 2 + 550.f,
		GameProcess::_windowInfo.height / 2 + 450.f,
		GameProcess::_windowInfo.height / 2 + 550.f);
	noButton->GetComponent<Button>()->screenResize = Vector2(1920.f, 1080.f);
	noButton->GetComponent<Button>()->Resize(GameProcess::resizeInfo.width, GameProcess::resizeInfo.height);
	noButton->GetComponent<Button>()->SetImages((int)ButtonStatus::NORMAL, L"pngwingcom.png");
	noButton->GetComponent<Button>()->SetImages((int)ButtonStatus::HIGHLIGHT, L"pngwingcom.png");
	noButton->GetComponent<Button>()->SetImages((int)ButtonStatus::PRESSED, L"pngwingcom.png");
	noButton->GetComponent<Button>()->SetIsAlpha(true);
	noButton->GetComponent<Button>()->SetAlpha(0.f);
	noButton->SetActive(false);
	AddGameObject(noButton);
	EventMachine::GetInstance()->SetNoButton(noButton);
	...
	competitorPanel1->GetComponent<Panel>()->SetSortLayer(1);
	...
	AddGameObject(competitorPanel1);
	EventMachine::GetInstance()->SetCompetitionPanel_1(competitorPanel1);

	// 경쟁자 대사
	shared_ptr<GameObject> competitorPanel2 = make_shared<GameObject>();
	...
	competitorPanel2->GetComponent<Panel>()->SetSortLayer(1);
	...
	AddGameObject(competitorPanel2);
	EventMachine::GetInstance()->SetCompetitionPanel_2(competitorPanel2);
	...
	uiManager->GetScript<UIManager>()->SetYesBtn(yesButton);
	uiManager->GetScript<UIManager>()->SetNoBtn(noButton);
