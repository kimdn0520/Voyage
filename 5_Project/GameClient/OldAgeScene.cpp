...
void OldAgeScene::Init()
{
	if (!DataManager::GetInstance()->isDataLoad)
		DataManager::GetInstance()->DataLoad();

	 // INTRO_END 에서 isAdulthoodIntro가 true가 되면 다음 이벤트로 넘어감
	EventMachine::GetInstance()->isOldAgeIntro = true;

	 // 테스트용 임시
	EventMachine::GetInstance()->ChangeState(EventMachine::OLDAGE_INTRO);
...
#pragma region UI
	...
	mainTalkPanel->AddComponent<OldAgeText>();
	...
	/*soundManager->GetScript<SoundManager>()->SetHeartGauge(heartGauge);*/
